...
def to_Sparsetensor(adjs):
    new_adjs=[]
    for i in range(len(adjs)):
        adj=adjs[i]
        indices=adj._indices()
        values=adj._values()
        new_adj=SparseTensor(row=indices[0], col=indices[1], value=values, sparse_sizes=(adj.size(0),adj.size(0)))
...
def normalize_adj_from_tensor(adj, mode, sparse=False,return_D=False):
    ...
    if not sparse:
        ...
        adj = adj.coalesce()
        if mode == "sym":
            inv_sqrt_degree = 1. / (torch.sqrt(torch.sparse.sum(adj, dim=1).values())+EPS)
        ...
        # new_values = adj.values()
        if return_D:
            return torch.sparse.FloatTensor(adj.indices(), new_values, adj.size()),D_value
        else:
...
def remove_self_loop_rcdd(adjs):
    adjs_ = []
    for adj in adjs:
        device = adj.device  # Store the original device
        adj = adj.cpu().coalesce()  # Move the tensor to CPU and coalesce it
        diag_index = torch.nonzero(adj.indices()[0] != adj.indices()[1]).flatten()
        adj = torch.sparse.FloatTensor(adj.indices()[:, diag_index], adj.values()[diag_index], adj.shape).coalesce()
        adjs_.append(adj)  # Move the tensor back to its original device
    return adjs_,device
...
def graph_process(adjs, feat, args):
    ...
    un_adjs=[adj+adj_I.to_sparse() for adj in adjs]
    ...
    return adjs_o,un_adjs

def graph_process_large(adjs, feat, args):
    adjs = [adj.coalesce() for adj in adjs]
    if args.dataset in ['rcdd','oag-cs','oag-eng']:
        adjs,device=remove_self_loop_rcdd(adjs)
        un_adjs = [sparse_tensor_add_self_loop(adj) for adj in adjs]
        adjs_o = [normalize_adj_from_tensor(adj, mode='sym', sparse=True) for adj in un_adjs]
        if args.dataset in ['oag-eng','oag-cs']:
            for i in range(len(adjs_o)):
                adjs_o[i]=adjs_o[i].to(device)
    else:
